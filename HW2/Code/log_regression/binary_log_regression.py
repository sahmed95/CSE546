...
class BinaryLogReg:
    ...
    def step(self, X: np.ndarray, y: np.ndarray, learning_rate: float = 1e-4):
        ...
        bias = self.bias
        self.bias = bias- learning_rate*self.gradient_J_bias(X,y)
        self.weight  = weight - learning_rate*self.gradient_J_weight(X,y)
...
if __name__ == "__main__":
    ...
    history1 = model.train(x_train, y_train, x_test, y_test, learning_rate = 0.1, epochs = 5000, batch_size = x_train.shape[0])
    history2 = model.train(x_train, y_train, x_test, y_test, learning_rate = 1e-4, batch_size = 1)
    history3 = model.train(x_train, y_train, x_test, y_test, learning_rate = 1e-3, epochs = 500)
