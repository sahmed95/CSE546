...
class LinearLayer(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.bias =nn.Parameter(torch.randn(dim_out,generator = generator, dtype=torch.float, requires_grad =True))
        self.weight = nn.Parameter(torch.randn((dim_in,dim_out), generator=generator, dtype=torch.float, requires_grad=True))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.add(prod,self.bias) 
